...
--                   * moodleMult * weatherMult * spotterMult
--                   * distanceMult * globalScalar
...
local function skillMultiplier(player)
    local level = 0
    if Config.SCATTER.useAimingSkill == false then return 1.0, level end
...
--   range             tiles to target (distance factor: closer = tighter)
...
function Scatter.computeRadius(opts)
    ...
    -- Distance factor: dispersion grows with range (closer = more accurate),
    -- interpolated across the mortar's full reach.
    local dMult = 1.0
    if opts.range and Config.SCATTER.distanceMinMult then
        local t = Mth.clamp(opts.range / math.max(Config.RANGE.maxRangeTiles, 1), 0, 1)
        dMult = Mth.lerp(Config.SCATTER.distanceMinMult, Config.SCATTER.distanceMaxMult, t)
    end
